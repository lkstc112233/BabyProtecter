...
public class JimmyActivity extends Activity implements PropertyChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(new JimmyView(this));
	...
	final SensorEventListener myListener = new SensorEventListener() {
		...
		public void onSensorChanged(SensorEvent event) {
			if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
				...
				if (sum > 200)
					JimmyAlarmer.playSound((int) sum);
			}
	...
	public void propertyChange(final PropertyChangeEvent event) {
		...
		runOnUiThread(new Runnable() {
			public void run() {
				...
				if (eventName.equals("NEW_DEVICE_ADVERTISING")) {
					...
					Measurements.INSTANCE.addPropertyChangeListener(JimmyActivity.this);
				}
			}
		});
		...
		if (property.equals(PROPERTY_ACCELEROMETER)) {
			...
			if (sum > 1.5)
				JimmyAlarmer.playSound((int) (sum * 100));
