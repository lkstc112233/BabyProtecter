...
//	private float storedRadius;
	...
	private float getAngle() {
		float d = getDistance();
		float x = xO - JimmyActivity.getActivity().phoneP[0];
		float y = xO - JimmyActivity.getActivity().phoneP[0];
	...
	private float getDistance() {
		return (float) Math.pow(
				Math.pow(xO - JimmyActivity.getActivity().phoneP[0], 2)
						+ Math.pow(yO - JimmyActivity.getActivity().phoneP[1], 2), 0.5) * 100;
	}
	...
	public RadioScanner(RelativePoint left_up, RelativePoint right_bottom) {
		...
		// storedRadius = 70;
		xO = JimmyActivity.getActivity().phoneP[0];
		yO = JimmyActivity.getActivity().phoneP[1];
	...
	public void draw(Canvas canvas) {
		float angle = -JimmyActivity.getActivity().phoneOrientationValues[0] - getAngle();
